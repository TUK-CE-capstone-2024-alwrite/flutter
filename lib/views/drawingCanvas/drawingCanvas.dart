...
class DrawingCanvas extends HookWidget {
  ...
  Widget build(BuildContext context) {
    return MouseRegion( //마우스 이벤트 감지 후 마우스 커서 변경
      cursor: SystemMouseCursors.precise, //precise 마우스 커서로 변경
      child: Stack( //여러 위젯을 겹쳐서 표시할 수 있는 위젯임
        children: [
  ...
  void onPointerDown(PointerDownEvent details, BuildContext context) {
    final box = context.findRenderObject() as RenderBox; // 캐스팅(포인터 이벤트가 발생한 위치를 포함하는 box 얻기)
    final offset = box.globalToLocal(details.position); //해당 box를 로컬 좌표계로 변환 후 화면상의 좌표를 box내의 좌표로 변환
    currentSketch.value = Sketch.fromDrawingMode( //변수에 따라 스케치 생성(그리기모드, 도형, 지우개 등등)
      Sketch(
        points: [offset],
  ...
  void onPointerUp(PointerUpEvent details) {
    allSketches.value = List<Sketch>.from(allSketches.value)
      ..add(currentSketch.value!); //allSketches.value 를 복사하여 새 리스트 만들고 그 리스트에 현재 스케치 추가
    currentSketch.value = Sketch.fromDrawingMode(
      Sketch(
        points: [],
        size: drawingMode.value == DrawingMode.eraser
  ...
  Widget buildAllSketches(BuildContext context) {
    return SizedBox(
      height: height,
      width: width,
      child: ValueListenableBuilder<List<Sketch>>( //allSketches값 감시, 값이 변경될때마다 화면을 다시 그림
        valueListenable: allSketches,
        builder: (context, sketches, _) {
          return RepaintBoundary( //자식 위젯의 그리기 영역을 따로 관리
  ...
  Widget buildCurrentPath(BuildContext context) {
    return Listener( //Listener 위젯으로 포인터 이벤트를 감지 하고 처리
